      ...
      @if ($navigation)
        @foreach ($navigation as $item)
          <li class="group" @mouseenter="menu=true " @mouseleave="menu = null">
            <a class="first:pl-0 px-3 lg:px-4 block flex lg:space-x-1 items-center relative hover:underline" href="{!! $item->url !!}">
              <span class="font-karbon text-sm lg:text-nav uppercase tracking-wider">{!! $item->label!!}</span> 
              @if ($item->children)
